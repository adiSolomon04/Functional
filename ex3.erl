...
rowElemMult(RowA, RowB) -> [A*B*1.0 || {A,B} <- lists:zip(RowA, RowB)].

%% lists:zip func builds a list of taples
%% where each taple is an item from the first list
%% and a corresponding item from the second list.
...
%% The calculations happen once for the n'th fibonacci item.